...
salud['tipo_trab1'] = np.where(salud['pea'] == 1, 
                                salud['tipo_trab1'], 
                                 salud['tipo_trab1']) # Depende de un patrón, jefe o superior 
...
salud['tipo_trab2'] = np.where(salud['pea'] == 1, 
                                salud['tipo_trab2'], 
                                 salud['tipo_trab2']) # Depende de un patrón, jefe o superior
...
salud.loc[((salud['atemed'] == 1) & ((salud['inst_1'] == 1) | 
           (salud['inst_2'] == 2) | (salud['inst_3'] == 3) | 
            (salud['inst_4'] == 4)) & (salud['inscr_6'] == 6) & 
             (salud['edad'] >= 12) & (~salud['edad'].isna())), 'smcv'] = 1 # Sí cuenta
...
##### Opción 1 {.tabset .tabset-pills .top4-tiles} 
...
seguridad_social['tipo_trab1'] = np.where(seguridad_social['pea'] == 1, 
                                           seguridad_social['tipo_trab1'], 
                                            seguridad_social['tipo_trab1']) # Depende de un patrón, jefe o superior 
...
seguridad_social['tipo_trab2'] = np.where(seguridad_social['pea'] == 1, 
                                           seguridad_social['tipo_trab2'], 
                                            seguridad_social['tipo_trab2']) # Depende de un patrón, jefe o superior
...
seguridad_social.loc[((seguridad_social['trabajo_mp'] == 2) & ((seguridad_social['act_pnea1'] == 2) | 
                      (seguridad_social['act_pnea2'] == 2))), 'jub'] = 1 # Población pensionada o jubilada
seguridad_social.loc[((seguridad_social['ing_pens'] > 0) & (~seguridad_social['ing_pens'].isna())), 'jub'] = 1 # Población pensionada o jubilada
seguridad_social.loc[(seguridad_social['inscr_2'] == 2), 'jub'] = 1 # Población pensionada o jubilada
seguridad_social.loc[(seguridad_social['jub'].isna()), 'jub'] = 0 # Población no pensionada o jubilada
...
seguridad_social.loc[(seguridad_social['ocupa1'] == 1), 'smlab1'] = 0 # Con servicios médicos
seguridad_social.loc[((seguridad_social['ocupa1'] == 1) & (seguridad_social['atemed'] == 1) & 
                      ((seguridad_social['inst_1'] == 1) | (seguridad_social['inst_2'] == 2) | 
                       (seguridad_social['inst_3'] == 3) | (seguridad_social['inst_4'] == 4)) & 
                        (seguridad_social['inscr_1'] == 1)), 'smlab1'] = 1 # Sin servicios médicos
## Ocupación secundaria
seguridad_social['smlab2'] = np.NaN
seguridad_social.loc[(seguridad_social['ocupa2'] == 1), 'smlab2'] = 0 # Con servicios médicos
seguridad_social.loc[((seguridad_social['ocupa2'] == 1) & (seguridad_social['atemed'] == 1) & 
                      ((seguridad_social['inst_1'] == 1) | (seguridad_social['inst_2'] == 2) | 
                       (seguridad_social['inst_3'] == 3) | (seguridad_social['inst_4'] == 4)) & 
                        (seguridad_social['inscr_1'] == 1)), 'smlab2'] = 1 # Sin servicios médicos
...
seguridad_social.loc[((seguridad_social['atemed'] == 1) & ((seguridad_social['inst_1'] == 1) |
                      (seguridad_social['inst_2'] == 2) | (seguridad_social['inst_3'] == 3) |
                       (seguridad_social['inst_4'] == 4)) & (seguridad_social['inscr_6'] == 6) & 
                        ((seguridad_social['edad'] >= 12) & (~seguridad_social['edad'].isna()))), 'smcv'] = 1 # Sí cuenta
...
seguridad_social.loc[((seguridad_social['segvol_1'] == 1) & ((seguridad_social['edad'] >= 12) & 
                      (~seguridad_social['edad'].isna()))), 'aforecv'] = 1 # Sí cuenta 
...
seguridad_social.loc[((seguridad_social['parentesco'] >= 200) & (seguridad_social['parentesco'] < 300)), 'par'] = 2 # Cónyuge del  jefe/a
...
seguridad_social.loc[((seguridad_social['asis_esc'] == 1)), 'inas_esc'] = 0 # Sí asiste
...
seguridad_social.loc[(seguridad_social['cony_ss'] > 0), 'cony_ss'] = 1 # Acceso directo a servicios de salud del cónyuge de la jefatura del hogar
...
# Otros núcleos familiares: se identifica a la población con acceso a la seguridad social mediante otros núcleos familiares a través de la afiliación o inscripción 
#a servicios de salud por algún familiar dentro o fuera del hogar, muerte del asegurado o por contratación propia.
...
##### Opción 2 {.tabset .tabset-pills .top4-tiles} 
...
```{python}
# Definiendo las constantes de deflación
deflactores = {
...
def indicador_pensiones(pensiones):
    ...
    # Función para deflactar los ingresos
    ...
    # Función para calcular los ingresos promedios
    ...
    # Función para agrupar los datos por hogar y sumar los ingresos
...
def indicador_seguridad_social(poblacion):
    ...
    def integrar_bases(df):
       ...
       # Convirtiendo variables string a numéricas
    def convertir_a_numerico(df):
...
#### V. Indicador de carencia por acceso a los servicios básicos de la vivienda {.tabset .tabset-pills .top3-tiles}    
...
#### VI. Indicador de carencia por acceso a la alimentación nutritiva y de calidad {.tabset .tabset-pills .top3-tiles}  
...
# Indicador de hogares con menores_alimentacion de 18 años
...
alimentacion.loc[(alimentacion['acc_alim4'] == 1), 'ia_1ad'] = 1 # Sí
...
alimentacion.loc[(alimentacion['acc_alim5'] == 1), 'ia_2ad'] = 1 # Sí
...
alimentacion.loc[(alimentacion['acc_alim6'] == 1), 'ia_3ad'] = 1 # Sí
...
alimentacion.loc[(alimentacion['acc_alim2'] == 1), 'ia_4ad'] = 1 # Sí
...
alimentacion.loc[(alimentacion['acc_alim7'] == 1), 'ia_5ad'] = 1 # Sí
...
alimentacion.loc[(alimentacion['acc_alim8'] == 1), 'ia_6ad'] = 1 # Sí    
...
alimentacion.loc[(alimentacion['acc_alim11'] == 1), 'ia_7men'] = 1 # Sí 
...
alimentacion.loc[(alimentacion['acc_alim12'] == 1), 'ia_8men'] = 1 # Sí 
...
alimentacion.loc[(alimentacion['acc_alim13'] == 1), 'ia_9men'] = 1 # Sí 
...
alimentacion.loc[(alimentacion['acc_alim14'] == 1), 'ia_10men'] = 1 # Sí 
...
alimentacion.loc[(alimentacion['acc_alim15'] == 1), 'ia_11men'] = 1 # Sí 
...
alimentacion.loc[(alimentacion['acc_alim16'] == 1), 'ia_12men'] = 1 # Sí 
...
# Se genera el indicador de carencia por acceso a la alimentación que considera en situación de carencia a la población en hogares que presenten inseguridad alimentaria moderada o severa
...
# Puntaje total de consumo ponderado de alimentos, indica el número ponderado de grupos de alimentos que se consumieron en los últimos siete días.   
...
alimentacion.loc[((alimentacion['tot_cpond'] > 28) &
                  (alimentacion['tot_cpond'] <= 42)), 'dch'] = 2 # Limítrofe
...
def indicador_alimentacion(hogares):
    ...
    # Función para calcular los totales de inseguridad alimentaria
    ...
    # Función para calcular el grado de inseguridad alimentaria
    ...
    # Función para calcular el indicador de carencia por acceso a la alimentación
    def calcular_indicador_carencia(df):
        df['ic_ali'] = np.NaN
...
def indicador_alimentacion_anexoa(alimentacion):
    # Función para calcular los diferentes valores de cpond
    ...
    # Función para calcular el puntaje total de consumo ponderado de alimentos
    ...
    # Función para categorizar la dieta consumida en los hogares
    def categorizar_dieta(df):
        df['dch'] = np.NaN
        df.loc[df['tot_cpond'].between(0, 28), 'dch'] = 1  # Pobre
        df.loc[(df['tot_cpond'] > 28) & (df['tot_cpond'] <= 42), 'dch'] = 2  # Limítrofe
        df.loc[(df['tot_cpond'] > 42) & (~df['tot_cpond'].isna()), 'dch'] = 3  # Aceptable
        
        return df
    
    # Función para calcular la limitación en el consumo de alimentos
...
def indicador_alimentacion_anexob(alimentacion):
      # Función para calcular la columna ic_ali_nc
    ...
    # Función para seleccionar las columnas finales
...
#### VII. Bienestar económico (ingresos) {.tabset .tabset-pills .top3-tiles}   
...
aguinaldos['trab'] = 1 # Población con al menos un empleo 
...
# En el caso de la información de gasto no monetario, para deflactar se utiliza la decena de levantamiento de la encuesta, 
# la cual se encuentra en la octava posición del folio de la vivienda. En primer lugar se obtiene una variable que 
...
def indicador_aguinaldos(trabajos):
    ...
    def agregar_columnas_aguinaldo(df):
        df['trab'] = 1  # Población con al menos un empleo 
...
Se procede a deflactar el ingreso recibido por los hogares a precios de agosto de 2022. Para ello, se utilizan las variables meses, las cuales toman los valores 2 a 10 e indican el mes en que se recibió el ingreso respectivo.   
...
def indicador_ingreso_deflactado(ingresos):
    ...
    def convertir_a_numerico(df):
        ...
        # Función auxiliar para deflactar ingresos
    def deflactar_ingreso(df, col_ingreso, col_mes):
        ...
        # Función auxiliar para deflactar ingresos
        # Aplicar la función a cada columna de ingreso
    ...
    #    """Aplica los deflactores a los ingresos según el mes correspondiente."""
...
def indicador_ingreso_deflactado_anexoa(ingreso_deflactado):
    def deflactar_ing_1(df):
        """Aplica deflactores específicos a ing_1 según la clave."""
    ...
    def calcular_ingreso_mensual(df):
        """Calcula el ingreso mensual promedio en los últimos seis meses."""
        df['ing_mens'] = df[['ing_1', 'ing_2', 'ing_3', 'ing_4', 'ing_5', 'ing_6']].mean(axis = 1, skipna = True)
        
        return df
    
    def clasificar_ingresos(df):
        """Clasifica los ingresos en categorías específicas."""
...
Se cambia la estructura de los deflactores del código anterior para reducir el cálculo como el chunk de la  `Opción 1`.
Se seleccionan los deflactores que solamente pertencen al cálculo de los rubros:    

```{python}
# Definición de los deflactores
deflactores = {
    'ali_nm': [1.0000000000, 1.0130754464, 1.0178275200, 1.0207468579, 1.0000000000],
    'alta_nm': [1.0000000000, 1.0035071112, 1.0111808568, 1.0131982216, 1.0000000000],
    ...
    'tpub_nm': [1.0000000000, 1.0034865488, 1.0052385833, 1.0064912880, 1.0000000000],
...
def indicador_no_monetario(gastoshogar):
    ...
    def deflactar_rubros(df):
        ...
        # Función auxiliar para deflactar según los deflactores y la decena
...
def indicador_pagos_especie(no_monetario):
    # Función para filtrar pagos en especie
    def filtrar_pagos_especie(df):
        return df[df['esp'] == 1]

    # Función para agrupar y sumar gastos no monetarios
    def agrupar_y_sumar(df):
    ...
    # Construcción de la base de pagos en especie utilizando pipe
...
```{python}
def indicador_regalos(no_monetario):

    # Función para filtrar regalos
    def filtrar_regalos(df):
        return df[df['reg'] == 1]
    ...
    def agrupar_y_sumar(df):
        columnas_gasto = ['gasto_nm', 'ali_nm', 'alta_nm', 'veca_nm', 'viv_nm', 'lim_nm', 'cris_nm', 'ens_nm', 
                          'sal_nm', 'tpub_nm', 'tfor_nm', 'com_nm', 'edre_nm', 'edba_nm', 'cuip_nm', 'accp_nm', 
                          'otr_nm', 'reda_nm']
        df = df.groupby(['folioviv', 'foliohog'])[columnas_gasto
                                                 ].sum(numeric_only = True).reset_index()
        return df
    
    # Función para renombrar las columnas
    def renombrar_columnas_regalos(df):
        nuevas_columnas = ['folioviv', 'foliohog', 'gasto_nm', 'ali_nmr', 'alta_nmr', 'veca_nmr', 'viv_nmr', 'lim_nmr', 
                           'cris_nmr', 'ens_nmr', 'sal_nmr', 'tpub_nmr', 'tfor_nmr', 'com_nmr', 'edre_nmr', 'edba_nmr', 
                           'cuip_nmr', 'accp_nmr', 'otr_nmr', 'reda_nmr']
        df.columns = nuevas_columnas
        return df
    
    # Construcción de la base de regalos utilizando pipe
    return (no_monetario.copy()
                        .pipe(filtrar_regalos)
                        .pipe(agrupar_y_sumar)
                        .pipe(renombrar_columnas_regalos))

regalos = indicador_regalos(no_monetario)
```

**Se guarda la base de datos**
```{python}
regalos.to_csv(current_directory + '/Output/Data/' + '/reg_def22_2.csv', index = False)
```
...
```{python}
def ingreso_corriente (concentradohogar):
    
    # Función para seleccionar columnas iniciales
    def seleccionar_columnas_iniciales(df):
        # Lista de columnas iniciales
        columnas = ['folioviv', 'foliohog', 'tam_loc', 'factor', 'tot_integ', 'est_dis', 'upm', 'ubica_geo']
        return df[columnas]
    
        # Función para incorporar bases de datos mediante merge
    def incorporar_base(df, base):
        return pd.merge(df, 
                         base, 
                          on = ['folioviv', 'foliohog'], 
                           how = 'left')
     
    # Función para determinar si es rural o urbano
    def determinar_rural_urbano(df):
        df.loc[df['tam_loc'] == 4, 'rururb'] = 1 # Rural
        df.loc[df['tam_loc'] <= 3, 'rururb'] = 0 # Urbano
        return df
    
    # Función para calcular ingreso corriente no monetario pago en especie
    def calcular_pago_especie(df):
        columnas_pago_esp = ['ali_nme', 'alta_nme', 'veca_nme', 'viv_nme', 'lim_nme', 'cris_nme', 'ens_nme', 'sal_nme',
                             'tpub_nme', 'tfor_nme', 'com_nme', 'edre_nme', 'cuip_nme', 'accp_nme', 'otr_nme']
        df['pago_esp'] = df[columnas_pago_esp].sum(axis = 1)
        return df
    
    # Función para calcular ingreso corriente no monetario regalos en especie
    def calcular_regalos_especie(df):
        columnas_reg_esp = ['ali_nmr', 'alta_nmr', 'veca_nmr', 'viv_nmr', 'lim_nmr', 'cris_nmr', 'ens_nmr', 'sal_nmr',
                            'tpub_nmr', 'tfor_nmr', 'com_nmr', 'edre_nmr', 'cuip_nmr', 'accp_nmr', 'otr_nmr']
        df['reg_esp'] = df[columnas_reg_esp].sum(axis = 1)
        return df
    
    # Función para calcular ingreso corriente no monetario total
    def calcular_ingreso_no_monetario(df):
        df['nomon'] = df[['pago_esp', 'reg_esp']].sum(axis = 1)
        return df
    
    # Función para calcular ingreso corriente total
    def calcular_ingreso_corriente_total(df):
        df['ict'] = df[['ing_mon', 'nomon']].sum(axis = 1)
        return df
    
    # Función para seleccionar columnas finales
    def seleccionar_columnas_finales(df):
        columnas_finales = ['folioviv', 'foliohog', 'ubica_geo', 'tam_loc', 'est_dis', 'upm', 'factor', 'tot_integ', 'ing_mon',
                            'ing_lab', 'ing_ren', 'ing_tra', 'ali_nme', 'alta_nme', 'veca_nme', 'viv_nme', 'lim_nme', 'cris_nme',
                            'ens_nme', 'sal_nme', 'tpub_nme', 'tfor_nme', 'com_nme', 'edre_nme', 'edba_nme', 'cuip_nme', 'accp_nme',
                            'otr_nme', 'reda_nme', 'ali_nmr', 'alta_nmr', 'veca_nmr', 'viv_nmr', 'lim_nmr', 'cris_nmr', 'ens_nmr',
                            'sal_nmr', 'tpub_nmr', 'tfor_nmr', 'com_nmr', 'edre_nmr', 'edba_nmr', 'cuip_nmr', 'accp_nmr', 'otr_nmr',
    ...
    # Construcción de la base ingreso_corriente utilizando pipe
    return (concentradohogar.copy()
                            .pipe(seleccionar_columnas_iniciales)
                            .pipe(incorporar_base, ingreso_deflactado)
                            .pipe(incorporar_base, pagos_especie)
                            .pipe(incorporar_base, regalos)
                            .pipe(determinar_rural_urbano)
                            .pipe(calcular_pago_especie)
                            .pipe(calcular_regalos_especie)
                            .pipe(calcular_ingreso_no_monetario)
                            .pipe(calcular_ingreso_corriente_total)
                            .pipe(seleccionar_columnas_finales))

ingreso_corriente = ingreso_corriente (concentradohogar)
```


**Se guarda la base de datos**
```{python}
ingreso_corriente.to_csv(current_directory + '/Output/Data/' + '/ingresotot22_2.csv', index = False)
```

#### Construcción del tamaño de hogar con economías de escala y escalas de equivalencia {.tabset .tabset-pills .top2-tiles}  

##### Opción 1 {.tabset .tabset-pills .top3-tiles}  
...
##### Opción 2 {.tabset .tabset-pills .top3-tiles}  

```{python}
def tam_hogar_escala(poblacion): 

    # Función para filtrar la población objetivo
    def filtrar_poblacion_objetivo(df):
        return df.loc[~((df['parentesco'] >= 400) & (df['parentesco'] < 500) |
                        (df['parentesco'] >= 700) & (df['parentesco'] < 800))]
    
    # Función para añadir la columna de total de integrantes del hogar
    def agregar_total_integrantes(df):
        df['ind'] = 1
        df['tot_ind'] = df.groupby(['folioviv', 'foliohog'])['ind'].transform('sum')
        return df
    
    # Función para agregar columnas de escalas de equivalencia
    def agregar_escalas_equivalencia(df):
        df['n_05'] = ((df['edad'] >= 0) & (df['edad'] <= 5)).astype(int)
        df['n_6_12'] = ((df['edad'] >= 6) & (df['edad'] <= 12)).astype(int)
        df['n_13_18'] = ((df['edad'] >= 13) & (df['edad'] <= 18)).astype(int)
        df['n_19'] = (df['edad'] >= 19).astype(int)
        return df
    
    # Función para calcular la escala del tamaño del hogar
    def calcular_tamhogesc(df):
        df['tamhogesc'] = 0
        df.loc[df['n_05'] == 1, 'tamhogesc'] = df['n_05'] * .7031
        df.loc[df['n_6_12'] == 1, 'tamhogesc'] = df['n_6_12'] * .7382
        df.loc[df['n_13_18'] == 1, 'tamhogesc'] = df['n_13_18'] * .7057
        df.loc[df['n_19'] == 1, 'tamhogesc'] = df['n_19'] * .9945
        df.loc[df['tot_ind'] == 1, 'tamhogesc'] = 1
        return df
    
    # Función para agrupar por folios y sumar tamhogesc
    def agrupar_y_sumar_tamhogesc(df):
        return df.groupby(['folioviv', 'foliohog'])['tamhogesc'].sum().reset_index()
    
    # Aplicar funciones en cadena utilizando pipe
    return (poblacion.copy()
                     .pipe(filtrar_poblacion_objetivo)
                     .pipe(agregar_total_integrantes)
                     .pipe(agregar_escalas_equivalencia)
                     .pipe(calcular_tamhogesc)
                     .pipe(agrupar_y_sumar_tamhogesc))

tam_hogar_escala = tam_hogar_escala (poblacion)
```

**Se guarda la base de datos**
```{python}
tam_hogar_escala.to_csv(current_directory + '/Output/Data/' + '/tamhogesc22_2.csv', index = False)
```

#### Bienestar económico {.tabset .tabset-pills .top2-tiles}  
...
```{python}
def p_ingreso(ingreso_corriente):
    # Función para incorporar el tamaño del hogar ajustado
    def merge_tam_hogar(df, tam_hogar_escala):
        return pd.merge(df, 
                         tam_hogar_escala, 
                          on = ['folioviv', 'foliohog'], 
                           how = 'left')
    
    # Función para calcular información per cápita
    def calcular_ictpc(df):
        df['ictpc'] = df['ict'] / df['tamhogesc']
        return df
    
    # Función para identificar hogares bajo la línea de pobreza extrema
    def identificar_plpe(df):
        df['plp_e'] = 0
        df.loc[(df['ictpc'] < lp1_urb ) & (df['rururb'] == 0), 'plp_e'] = 1
        df.loc[(df['ictpc'] < lp1_rur) & (df['rururb'] == 1), 'plp_e'] = 1
        return df
    
    # Función para identificar hogares bajo la línea de pobreza
    def identificar_plp(df):
        df['plp'] = 0
        df.loc[(df['ictpc'] < lp2_urb) & (df['rururb'] == 0), 'plp'] = 1
        df.loc[(df['ictpc'] < lp2_rur) & (df['rururb'] == 1), 'plp'] = 1
        return df
    
    # Función para seleccionar las columnas finales
    def seleccionar_columnas(df):
        columnas = ['folioviv', 'foliohog', 'factor', 'tam_loc', 'rururb', 'tamhogesc', 'ict', 'ictpc',
                    'plp_e', 'plp', 'est_dis', 'upm', 'ubica_geo', 'tot_integ', 'ing_mon', 'ing_lab',
                    'ing_ren', 'ing_tra', 'nomon', 'pago_esp', 'reg_esp']
        return df[columnas]
    
    # Aplicar funciones en cadena utilizando pipe
    return (ingreso_corriente.copy()
                             .pipe(merge_tam_hogar, tam_hogar_escala)
                             .pipe(calcular_ictpc)
                             .pipe(identificar_plpe)
                             .pipe(identificar_plp)
                             .pipe(seleccionar_columnas))

p_ingreso = p_ingreso(ingreso_corriente)
```


**Se guarda la base de datos**
```{python}
p_ingreso.to_csv(current_directory + '/Output/Data/' + '/p_ingresos22_2.csv', index = False)
```

## Pobreza multidimensional  {.tabset .tabset-pills .top3-tiles} 

### Integración de las bases {.tabset .tabset-pills .top4-tiles}  
...
```{python}
def indicador_pobreza(rezago_educativo):
    # Función para realizar los merges necesarios
    def realizar_merges(df, salud, seguridad_social, calidad_viviendas, servicios_basicos, alimentacion, p_ingreso):
        df = pd.merge(df, salud, on = ['folioviv', 'foliohog', 'numren'], how = 'left')
        df = pd.merge(df, seguridad_social, on = ['folioviv', 'foliohog', 'numren'], how = 'left')
        df = pd.merge(df, calidad_viviendas, on = ['folioviv', 'foliohog'], how = 'left')
        df = pd.merge(df, servicios_basicos, on = ['folioviv', 'foliohog'], how = 'left')
        df = pd.merge(df, alimentacion, on = ['folioviv', 'foliohog'], how = 'left')
        df = pd.merge(df, p_ingreso, on = ['folioviv', 'foliohog'], how = 'left')
        return df
    
    # Función para rellenar valores nulos en columnas específicas
    def rellenar_nulos(df):
        columnas = ['ing_mon', 'ing_lab', 'ing_ren', 'ing_tra']
        for col in columnas:
    ...
    def eliminar_duplicados(df):
        return df.drop_duplicates(subset=['folioviv', 'foliohog', 'numren'], keep = 'first')
    
    # Función para ajustar formato de folioviv y mapear entidades
    def ajustar_formato_entidades(df):
        df['folioviv'] = df['folioviv'].astype(str).str.zfill(10)
        df['ent'] = df['folioviv'].str[:2].astype(int)
        df['entidad'] = df['ent'].map(entidad)
        return df
    
    # Aplicar funciones en cadena utilizando pipe
    return (rezago_educativo.copy()  # DataFrame vacío inicial para el merge
                            .pipe(realizar_merges, salud, seguridad_social, calidad_viviendas, servicios_basicos, alimentacion, p_ingreso)
                            .pipe(rellenar_nulos)
                            .pipe(eliminar_duplicados)
                            .pipe(ajustar_formato_entidades))
              
pobreza = indicador_pobreza(rezago_educativo)
```


### Índice de Privación Social {.tabset .tabset-pills .top3-tiles} 
...
```{python}
# Función para calcular el índice de privación
def calcular_indice_privacion(df):
    df['i_privacion'] = df[['ic_rezedu', 'ic_asalud', 'ic_segsoc', 'ic_cv', 'ic_sbv', 'ic_ali_nc']].sum(axis = 1)
    return df

# Función para manejar los valores nulos en el índice de privación
def manejar_nulos_indice_privacion(df):
    condiciones = [
        df['ic_rezedu'].isna(),
        df['ic_asalud'].isna(),
        df['ic_segsoc'].isna(),
    ...
    df.loc[np.any(condiciones, axis=0), 'i_privacion'] = np.NaN
    return df

# Aplicar funciones en cadena utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(calcular_indice_privacion)
                  .pipe(manejar_nulos_indice_privacion))
```
...
```{python}
# Función para clasificar pobreza
def clasificar_pobreza(df):
    condiciones_pobreza = (df['plp'] == 1) & (df['i_privacion'] >= 1) & (~df['i_privacion'].isna())
    df.loc[condiciones_pobreza, 'pobreza'] = 1
    condiciones_no_pobreza = ((df['plp'] == 0) | (df['i_privacion'] == 0)) & (~df['plp'].isna()) & (~df['i_privacion'].isna())
    df.loc[condiciones_no_pobreza, 'pobreza'] = 0
    return df

# Función para clasificar pobreza extrema
def clasificar_pobreza_extrema(df):
    condiciones_extrema = (df['plp_e'] == 1) & (df['i_privacion'] >= 3) & (~df['i_privacion'].isna())
    df.loc[condiciones_extrema, 'pobreza_e'] = 1
    condiciones_no_extrema = ((df['plp_e'] == 0) | (df['i_privacion'] < 3)) & (~df['plp_e'].isna()) & (~df['i_privacion'].isna())
    df.loc[condiciones_no_extrema, 'pobreza_e'] = 0
    return df

# Función para clasificar pobreza moderada
def clasificar_pobreza_moderada(df):
    condiciones_moderada = (df['pobreza'] == 1) & (df['pobreza_e'] == 0)
    df.loc[condiciones_moderada, 'pobreza_m'] = 1
    condiciones_no_moderada = (df['pobreza'] == 0) | ((df['pobreza'] == 1) & (df['pobreza_e'] == 1))
    df.loc[condiciones_no_moderada, 'pobreza_m'] = 0
    return df

# Aplicar funciones en cadena utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(clasificar_pobreza)
                  .pipe(clasificar_pobreza_extrema)
                  .pipe(clasificar_pobreza_moderada))
```


### Población vulnerable {.tabset .tabset-pills .top3-tiles}  
...
```{python}
# Función para clasificar vulnerabilidad por carencias
def clasificar_vulnerabilidad_carencias(df):
    df['vul_car'] = 0
    condiciones_vulnerable = (df['plp'] == 0) & (df['i_privacion'] >= 1) & (~df['i_privacion'].isna())
    df.loc[condiciones_vulnerable, 'vul_car'] = 1
    df.loc[pd.isna(df['pobreza']), 'vul_car'] = pd.NA
    return df

# Función para clasificar vulnerabilidad por ingresos
def clasificar_vulnerabilidad_ingresos(df):
    df['vul_ing'] = 0
    condiciones_vulnerable = (df['plp'] == 1) & (df['i_privacion'] == 0)
    df.loc[condiciones_vulnerable, 'vul_ing'] = 1
    df.loc[pd.isna(df['pobreza']), 'vul_ing'] = pd.NA
    return df

# Aplicar funciones en cadena utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(clasificar_vulnerabilidad_carencias)
                  .pipe(clasificar_vulnerabilidad_ingresos))
```

### Población no pobre y no vulnerable {.tabset .tabset-pills .top3-tiles}  
...
```{python}
# Función para clasificar población no pobre y no vulnerable
def clasificar_no_pobre_no_vulnerable(df):
    df['no_pobv'] = 0
    condiciones_no_vulnerable = (df['plp'] == 0) & (df['i_privacion'] == 0)
    df.loc[condiciones_no_vulnerable, 'no_pobv'] = 1
    df.loc[pd.isna(df['pobreza']), 'no_pobv'] = pd.NA
    return df
  
# Aplicar funciones en cadena utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(clasificar_no_pobre_no_vulnerable))
```

### Población con carencias sociales {.tabset .tabset-pills .top3-tiles}  
...
pobreza.loc[(pobreza['i_privacion'] >= 1) &
             (~pobreza['i_privacion'].isna()), 'carencias'] = 1 # Población con al menos una carencia social
...
pobreza.loc[(pobreza['i_privacion'] >= 3) &
             (~pobreza['i_privacion'].isna()), 'carencias3'] = 1 # Población con al menos tres carencias sociales
...
```{python}
# Función para clasificar población con al menos una carencia
def clasificar_al_menos_una_carencia(df):
    df['carencias'] = 0
    condiciones_con_carencias = (df['i_privacion'] >= 1) & (~df['i_privacion'].isna())
    df.loc[condiciones_con_carencias, 'carencias'] = 1
    df.loc[pd.isna(df['pobreza']), 'carencias'] = pd.NA
    return df

# Función para clasificar población con tres o más carencias
def clasificar_tres_o_mas_carencias(df):
    df['carencias3'] = 0
    condiciones_con_carencias3 = (df['i_privacion'] >= 3) & (~df['i_privacion'].isna())
    df.loc[condiciones_con_carencias3, 'carencias3'] = 1
    df.loc[pd.isna(df['pobreza']), 'carencias3'] = pd.NA
    return df

# Aplicar funciones en cadena utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(clasificar_al_menos_una_carencia)
                  .pipe(clasificar_tres_o_mas_carencias))
```
...
```{python}
def asignar_cuadrantes(df):
    df['cuadrantes'] = np.NaN
    condiciones_pobres = (df['plp'] == 1) & (df['i_privacion'] >= 1) & (~df['i_privacion'].isna())
    condiciones_vulnerables_carencias = (df['plp'] == 0) & (df['i_privacion'] >= 1) & (~df['i_privacion'].isna())
    condiciones_vulnerables_ingresos = (df['plp'] == 1) & (df['i_privacion'] == 0)
    condiciones_no_pobres_no_vulnerables = (df['plp'] == 0) & (df['i_privacion'] == 0)
    
    df.loc[condiciones_pobres, 'cuadrantes'] = 1  # Pobres
    df.loc[condiciones_vulnerables_carencias, 'cuadrantes'] = 2  # Vulnerables por carencias
    df.loc[condiciones_vulnerables_ingresos, 'cuadrantes'] = 3  # Vulnerables por ingresos
    df.loc[condiciones_no_pobres_no_vulnerables, 'cuadrantes'] = 4  # No pobres y no vulnerables
    
    return df

# Aplicar la función utilizando pipe
pobreza = (pobreza.copy()
                 .pipe(asignar_cuadrantes))
```


```{python}
# Función auxiliar para calcular la distancia normalizada del ingreso respecto a la línea de pobreza por ingresos
def calcular_distancia_pobreza_ingresos(df, lp_rur, lp_urb):
    condiciones_rur = (df['rururb'] == 1) & (df['plp'] == 1)
    condiciones_urb = (df['rururb'] == 0) & (df['plp'] == 1)
    df['prof1'] = np.NaN
    df.loc[condiciones_rur, 'prof1'] = (lp_rur - df['ictpc']) / lp_rur
    df.loc[condiciones_urb, 'prof1'] = (lp_urb - df['ictpc']) / lp_urb
    df.loc[df['prof1'].isna() & df['ictpc'].notna(), 'prof1'] = 0
    return df

# Función auxiliar para calcular la distancia normalizada del ingreso respecto a la línea de pobreza extrema por ingresos
def calcular_distancia_pobreza_extrema_ingresos(df, lp1_rur, lp1_urb):
    condiciones_rur = (df['rururb'] == 1) & (df['plp_e'] == 1)
    condiciones_urb = (df['rururb'] == 0) & (df['plp_e'] == 1)
    df['prof_e1'] = np.NaN
    df.loc[condiciones_rur, 'prof_e1'] = (lp1_rur - df['ictpc']) / lp1_rur
    df.loc[condiciones_urb, 'prof_e1'] = (lp1_urb - df['ictpc']) / lp1_urb
    df.loc[df['prof_e1'].isna() & df['ictpc'].notna(), 'prof_e1'] = 0
    return df

# Aplicar las funciones utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(calcular_distancia_pobreza_ingresos, lp2_rur, lp2_urb)
                  .pipe(calcular_distancia_pobreza_extrema_ingresos, lp1_rur, lp1_urb))
```

### Profundidad de la privación social {.tabset .tabset-pills .top3-tiles}  
...
```{python}
# Función auxiliar para calcular la intensidad de la privación social
def calcular_intensidad_privacion(df):
    df['profun'] = df['i_privacion'] / 6
    
    # Intensidad de la privación social: pobres
    df['int_pob'] = df['profun'] * df['pobreza']
    
    # Intensidad de la privación social: pobres extremos
    df['int_pobe'] = df['profun'] * df['pobreza_e']
    
    # Intensidad de la privación social: población vulnerable por carencias
    df['int_vulcar'] = df['profun'] * df['vul_car']
    
    # Intensidad de la privación social: población con carencias sociales
    df['int_caren'] = df['profun'] * df['carencias']
...
# Aplicar la función utilizando pipe
pobreza = (pobreza.copy()
                  .pipe(calcular_intensidad_privacion)
                  .pipe(seleccionar_columnas))
```

**Se guarda la base de datos**
```{python}
pobreza.to_csv(current_directory + '/Output/Data/' + 'pobreza_22_2.csv', index = False)
```

## Resultados 

### Nivel nacional {.tabset .tabset-pills .top3-tiles} 
...
```{python}
def calcular_promedios_ponderados(df):
    base = df[~np.isnan(df['pobreza'])]
    indicadores = ['pobreza', 'pobreza_m', 'pobreza_e', 'vul_car', 'vul_ing', 'no_pobv', 
    ...
    for indicador in indicadores:
        porcentaje = np.average(base[indicador], weights = base['factor']) * 100
        total = np.average(base[base[indicador] == 1][indicador], 
                            weights = base[base[indicador] == 1]['factor'], 
                             returned=True)[1] / 1000000
        nac.append([indicador, porcentaje, total])
    
    return nac

# Aplicar la función utilizando pipe
nac = pobreza.pipe(calcular_promedios_ponderados)
...
### Porcentaje y número de personas por indicador de pobreza, entidad federativa. {.tabset .tabset-pills .top3-tiles}   
...
```{python}
# Función auxiliar para calcular promedios ponderados por entidad
def calcular_porcentajes_entidad(df):
    ids = np.unique(df['ent'])
    base = df[~np.isnan(df['pobreza'])]

    def promedio_ponderado(indicador, ent):
        sub_base = base[base['ent'] == ent]
        return np.average(sub_base[indicador], weights = sub_base['factor']) * 100

    porcentajes = [
        [promedio_ponderado(indicador, i) for i in ids]
        for indicador in ['pobreza', 'pobreza_m', 'pobreza_e', 'vul_car', 'vul_ing']
    ]

    pob_ent_por = list(map(list, zip(*porcentajes)))
    return pob_ent_por

# Función auxiliar para calcular totales por entidad
def calcular_totales_entidad(df):
    ids = np.unique(df['ent'])
    base = df[~np.isnan(df['pobreza'])]

    def total_ponderado(indicador, ent):
        sub_base = base[(base[indicador] == 1) & (base['ent'] == ent)]
        return np.average(sub_base[indicador], weights = sub_base['factor'], returned = True)[1]

    totales = [
        [total_ponderado(indicador, i) for i in ids]
        for indicador in ['pobreza', 'pobreza_m', 'pobreza_e', 'vul_car', 'vul_ing']
    ]

    pob_ent_tot = list(map(list, zip(*totales)))
    return pob_ent_tot

# Aplicar las funciones utilizando pipe
pob_ent_por = pobreza.pipe(calcular_porcentajes_entidad)
pob_ent_tot = pobreza.pipe(calcular_totales_entidad)

# Títulos para la tabla
titulos_por = ['pobreza', 'pobreza_m', 'pobreza_e', 'vul_car', 'vul_ing']
titulos_tot = ['pobreza', 'pobreza_m', 'pobreza_e', 'vul_car', 'vul_ing']

# Imprimir las tablas
print(tabulate(pob_ent_por, titulos_por, floatfmt = '.6f'))
print(tabulate(pob_ent_tot, titulos_tot, floatfmt = '.0f'))
```

### Porcentaje y número de personas por indicador de carencia social, entidad federativa. {.tabset .tabset-pills .top3-tiles}   
...
def calcular_porcentajes_entidad(df):
    ...
    indicadores = ['ic_rezedu', 'ic_asalud', 'ic_segsoc', 'ic_cv', 'ic_sbv', 'ic_ali_nc', 'carencias', 'carencias3', 'plp_e', 'plp']
    porcentajes = [
        [promedio_ponderado(indicador, i) for i in ids]
        for indicador in indicadores
    ]

    care_ent_por = list(map(list, zip(*porcentajes)))
    return care_ent_por
...
def calcular_totales_entidad(df):
    ...
    care_ent_tot = list(map(list, zip(*totales)))
    return care_ent_tot

# Aplicar las funciones utilizando pipe
care_ent_por = pobreza.pipe(calcular_porcentajes_entidad)
care_ent_tot = pobreza.pipe(calcular_totales_entidad)
